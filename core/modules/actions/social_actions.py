...
import random # Per scegliere un tipo di interazione se non specificato

# Importa Character solo per il type hinting
if TYPE_CHECKING:
    from core.character import Character # Evita import circolare con Character

from core.enums import NeedType, RelationshipType, SocialInteractionType
from core import settings
from .action_base import BaseAction

# --- Costanti specifiche di default per SocializeAction (a livello di modulo) ---
# L'effettivo valore sarà cercato prima nei parametri __init__, poi in settings, poi qui.
...
class SocializeAction(BaseAction):
    # ... (__init__, is_valid, on_start, execute_tick come definiti precedentemente) ...
    def __init__(self, 
                 npc: 'Character', 
                 target_npc: 'Character',
                 interaction_type: SocialInteractionType,
                 simulation_context: Optional['Simulation'] = None, # Aggiunto simulation_context
        ...
        _default_duration = _MODULE_DEFAULT_SOCIALIZE_CHAT_DURATION_TICKS 
        _description = f"Sta avendo una {interaction_type.display_name_it().lower()} con {target_npc.name if target_npc else 'sconosciuto'}."

        specific_duration_key = f"SOCIAL_ACT_{interaction_type.name}_DURATION_TICKS"
        module_default_specific_duration = _MODULE_DEFAULT_SOCIALIZE_CHAT_DURATION_TICKS
        ...
        elif interaction_type == SocialInteractionType.PROPOSE_INTIMACY: # Durata per la proposta
            module_default_specific_duration = getattr(settings, "SOCIAL_PROPOSE_INTIMACY_DURATION_TICKS", 10) # Es. 10 tick

        actual_duration = duration_ticks 
        if actual_duration is None:
            actual_duration = getattr(settings, specific_duration_key, None) 
            if actual_duration is None:
                actual_duration = module_default_specific_duration 
        ...
        # I guadagni specifici sono gestiti in on_finish
    ...
    def is_valid(self) -> bool:
        # Controlli di BaseAction (se ne avesse di generici, altrimenti True)
        ...
        # L'iniziatore vuole socializzare? (Bisogno Sociale non completamente soddisfatto)
        initiator_social_need = self.npc.get_need_value(NeedType.SOCIAL)
        if initiator_social_need is None: # Non dovrebbe succedere
            if settings.DEBUG_MODE: print(f"    [{self.action_type_name} VALIDATE - {self.npc.name}] Impossibile leggere bisogno SOCIAL.")
            return False
        if initiator_social_need >= (settings.NEED_MAX_VALUE - 5): # Non se > 95 (quasi pieno)
        ...
        # Il target è disponibile? (Non dorme, non in azione critica non interrompibile)
        if self.target_npc.current_action:
            if self.target_npc.current_action.action_type_name == "ACTION_SLEEP":
        ...
        # TODO: Controllare se sono nello stesso luogo (richiede sistema locazioni per NPC)
        
        # Validità specifica per tipo di interazione
        if self.interaction_type == SocialInteractionType.FLIRT:
            # L'iniziatore deve essere attratto dal genere del target
            ...
            # Il target deve essere potenzialmente attratto dal genere dell'iniziatore
            ...
            # Non flirtare con chi ha una relazione molto negativa
            relationship = self.npc.get_relationship_with(self.target_npc.npc_id)
            min_rel_score_for_flirt = getattr(settings, "MIN_REL_SCORE_FOR_FLIRT", -10) # Da settings
            ...
            # Si può litigare quasi con tutti, ma forse non se la relazione è estremamente positiva? O sì?
            # Per ora, la rendiamo sempre valida se le altre condizioni sono soddisfatte.
            pass

        # TODO: Logica di "accettazione" da parte del target (più avanzata)
        # Per ora, se l'iniziatore può e il target è disponibile, l'azione è valida.
    ...
    def on_start(self):
        ...
        # TODO: Il target potrebbe reagire e iniziare un'azione complementare (es. "Ascoltare")
        #       Per ora, il target è "passivo" ma riceverà effetti.

    def execute_tick(self):
        super().execute_tick()
        # TODO: Possibili effetti per-tick: piccoli cambiamenti di relazione, umore, ecc.
        #       L'interazione potrebbe anche fallire o terminare prima in base a eventi/reazioni.
        if self.is_started and settings.DEBUG_MODE:
            log_interval = max(1, self.duration_ticks // 3) # Logga circa 3 volte
    ...
    def on_finish(self):
        if not self.npc or not self.target_npc:
            super().on_finish() # Completa comunque l'azione per l'iniziatore
            return
        ...
        # Valori di base per guadagni/cambiamenti
        ...
        # --- Logica specifica per SocialInteractionType ---
        if self.interaction_type == SocialInteractionType.DEEP_CONVERSATION:
            ...
            if current_rel_initiator_to_target and (current_rel_initiator_to_target.type.name.count("FRIEND") > 0 or current_rel_initiator_to_target.type.name.count("ROMANTIC") > 0 or current_rel_initiator_to_target.type == RelationshipType.ACQUAINTANCE): # Esempio, da migliorare con helper in Enum
        ...
        # --- LOGICA SPECIFICA PER PROPOSE_INTIMACY ---
            ...
            # Il target NPC (self.target_npc) ora deve decidere.
            # Il suo Character.choose_action() gestirà self.pending_intimacy_proposal_from
            if hasattr(self.target_npc, 'pending_intimacy_proposal_from'):
                self.target_npc.pending_intimacy_proposal_from = self.npc.npc_id # Imposta il flag sul target
                if settings.DEBUG_MODE: print(f"        Flag 'pending_intimacy_proposal_from' impostato per {self.target_npc.name} da {self.npc.name}")
            else: # Fallback se l'attributo non esiste (non dovrebbe succedere se Character è aggiornato)
                if settings.DEBUG_MODE: print(f"        ATTENZIONE: {self.target_npc.name} non ha l'attributo 'pending_intimacy_proposal_from'.")

            # La proposta in sé potrebbe avere un piccolo impatto sociale/relazionale, ma l'azione principale
            # (EngageIntimacyAction) avrà gli effetti maggiori se accettata.
            # Per ora, la proposta stessa non cambia i bisogni INTIMACY, solo un po' il SOCIAL e la relazione.
            initiator_social_gain = base_social_gain * 0.5 # Meno guadagno per una semplice proposta
            target_social_gain = base_social_gain * 0.2 # Il target non ha ancora fatto nulla
            relationship_score_change = base_rel_change * 0.5 # Piccolo impatto

        # --- FINE LOGICA SPECIFICA PER PROPOSE_INTIMACY ---

        # Applica guadagno ai bisogni SOCIALI (se non è una proposta di intimità che li gestisce diversamente)
        if self.interaction_type != SocialInteractionType.PROPOSE_INTIMACY:
            ...
            self.target_npc.change_need_value(NeedType.SOCIAL, target_social_gain)
            if settings.DEBUG_MODE:
                print(f"        Guadagno SOCIAL per {self.npc.name}: {initiator_social_gain:.1f}, per {self.target_npc.name}: {target_social_gain:.1f}")
        elif self.interaction_type == SocialInteractionType.PROPOSE_INTIMACY: # Per la proposta, un piccolo gain sociale
            self.npc.change_need_value(NeedType.SOCIAL, initiator_social_gain)
            # Non applichiamo social gain al target per una semplice proposta ricevuta,
            # la sua reazione determinerà ulteriori cambiamenti.
            if settings.DEBUG_MODE:
                print(f"        Guadagno SOCIAL (proposta) per {self.npc.name}: {initiator_social_gain:.1f}")


        # Modificatore "Prima Impressione" (se non è una proposta di intimità e non è un litigio)
        if self.interaction_type not in {SocialInteractionType.ARGUE, SocialInteractionType.PROPOSE_INTIMACY}:
            ...
            if is_new_or_superficial_relation:
                first_impression_mod = random.randint(getattr(settings, "FIRST_IMPRESSION_MIN_MOD", -2), getattr(settings, "FIRST_IMPRESSION_MAX_MOD", 2)) # Ridotto un po'
        ...
        self.target_npc.update_relationship( # Anche il target aggiorna la sua visione
            target_npc_id=self.npc.npc_id, 
            new_type=new_relationship_type, # Assumiamo simmetria nel tipo per ora
            score_change=int(relationship_score_change)
        )
    ...
    def on_interrupt_effects(self):
        ...
        # Potrebbe applicare un piccolo malus alla relazione o al bisogno sociale
        if self.npc and self.target_npc:
            self.npc.update_relationship(self.target_npc.npc_id, self.npc.get_relationship_with(self.target_npc.npc_id).type if self.npc.get_relationship_with(self.target_npc.npc_id) else RelationshipType.ACQUAINTANCE, score_change= - (self.relationship_change_amount // 2) )
            self.target_npc.update_relationship(self.npc.npc_id, self.target_npc.get_relationship_with(self.npc.npc_id).type if self.target_npc.get_relationship_with(self.npc.npc_id) else RelationshipType.ACQUAINTANCE, score_change= - (self.relationship_change_amount // 2) )