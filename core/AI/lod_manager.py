...
if TYPE_CHECKING:
    from ..simulation import Simulation # Per type hinting
    from ..character import Character # Per type hinting

class LODManager:
    HIGH_DETAIL_RADIUS = 50  # Unità di gioco (es. celle della griglia)
    MEDIUM_DETAIL_RADIUS = 150 # Esempio per un futuro livello MEDIUM
    ...
    def update_npc_lod(self, npc: 'Character', player_position: Tuple[float, float]):
        ...
        if distance < self.HIGH_DETAIL_RADIUS:
            new_lod = LODLevel.HIGH
        # elif distance < self.MEDIUM_DETAIL_RADIUS: # Per futuro livello MEDIUM
    ...
    def update_all_npcs_lod(self, player_position: Optional[Tuple[float, float]]):
        ...
        for npc_id, npc_instance in self.simulation.npcs.items():
