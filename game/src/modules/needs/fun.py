# simai/game/src/modules/needs/fun.py
...
try:
    ...
    print(f"CRITICAL ERROR (FunNeed): Could not import _base_need.BaseNeed or game_config: {e}")
...
class Fun(BaseNeed):
    def __init__(self, character_owner, max_value, 
        ...
        super().__init__(
            ...
            high_is_good=getattr(game_config, 'FUN_HIGH_IS_GOOD', True),
            name="Fun"
        )
        if DEBUG_VERBOSE:
            ...
            print(f"FUN_NEED_INIT ({owner_name}): Initialized. Value: {self.get_value():.2f}")