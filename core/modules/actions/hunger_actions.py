...
class EatAction(BaseAction):
    def __init__(self, 
        ...
        action_type_enum_val = ActionType.ACTION_EAT # Assicurati che ACTION_EAT esista nel tuo Enum ActionType
    ...
    def is_valid(self) -> bool:
        ...
        if not self.npc: return False # Controllo base da BaseAction o qui
        
        current_hunger = self.npc.get_need_value(NeedType.HUNGER)
        if current_hunger is None: return False

        # L'NPC mangia se non è completamente sazio.
        # settings.NEED_MAX_VALUE dovrebbe essere una costante globale affidabile.
        can_eat = current_hunger < settings.NEED_MAX_VALUE 
        
        if not can_eat and settings.DEBUG_MODE:
            print(f"    [{self.action_type_name} VALIDATE - {self.npc.name}] Non può mangiare, fame ({current_hunger:.1f}) già al massimo.")
        return can_eat
