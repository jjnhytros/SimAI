...
# %% auto 0
__all__ = ['DynamicalTrajectory', 'CircadianModel', 'Forger99', 'Hannay19', 'Hannay19TP', 
            'Jewett99', 'Hilaire07', 'Breslow13', 'Skeldon23']
...
class Breslow13(CircadianModel):
    ...
    def __init__(self, params=None):
        default_params = {
            'H1_threshold': 0.001, 
            'K': 0.55, # not specified in paper, taken from Jewett99 and Hilaire07
            'I0': 9500.0, 'I1': 100.0, 'alpha_0': 0.1, 'beta': 0.007,
            'p': 0.5, 'r': 15.36, 'G': 37.0, 'b': 0.4, 'gamma': 0.13,
            'kappa': 12.0/np.pi, 'tauc': 24.1, 'f': 0.99729, 'm': 7.0,
            'eta': 0.04, 'xi': 0.54, 
            # the following parameters have been converted from seconds to hours
        ...
        num_states = 6 # x, xc, n, H1, H2, H3
        num_inputs = 1 # light
        default_initial_condition = np.array([-0.219, -1.22, 0.519, 0.0, 0.0, 0.0])

        super(Breslow13, self).__init__(default_params, num_states, num_inputs, default_initial_condition)
        if params is not None:
            self.parameters = params
    ...
    def melatonin_drive(self, H2):
        exp_factor = 1.0 + np.exp((self.Hsat - H2)/ self.sigma)
        return self.Mmax / exp_factor
    

# %% ../nbs/api/00_models.ipynb 73
@patch_to(Breslow13)
def derv(self,
         t: float, # time
         state: np.ndarray, # dynamical state (x, xc, n, H1, H2, H3)
         input: np.ndarray # model input (light, wake)
         ) -> np.ndarray: # derivative of the state
     "Right-hand-side of the differential equation model"
     x = state[0,...]
     ...
     dydt[3,...] = - self.betaIP * H1 + A * (1.0 - self.m * B) * S_H1_B
     dydt[4,...] = self.betaIP * H1 - self.betaCP * H2 + self.betaAP * H3
     dydt[5,...] = - self.betaAP * H3

     return dydt

# %% ../nbs/api/00_models.ipynb 74
@patch_to(Breslow13)
def phase(self,
          trajectory: DynamicalTrajectory=None, # trajectory to calculate the phase. If None, the current trajectory is used
          time: float=None # a time point to calculate the phase at. If None, the phase is calculated for the entire trajectory
...
class Skeldon23(CircadianModel):
    ...
    def __init__(self, params=None):
        ...
        if params is not None:
            ...
            if 'forced_wakeup_weekday_only' in params:
                self.forced_wakeup_weekday_only = params['forced_wakeup_weekday_only']
            else:
                self.forced_wakeup_weekday_only = default_params['forced_wakeup_weekday_only']
        else:
    ...
    def step_rk4(self,
        ...
        if self.forced_wakeup_by_light:
            ...
            if self.forced_wakeup_weekday_only:
                ...
                is_weekday = True
            if (input > self.forced_wakeup_light_threshold) and is_weekday:
                new_sleep_state = 0.0
